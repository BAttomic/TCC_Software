...
import { ArrowLeft, TrendingUp, Ticket as TicketIcon, Users, DollarSign } from "lucide-react";
...
export default async function OrganizerAnalyticsPage({ params }: PageProps) {
  ...
  if (!event) notFound();
  if (session.user.role !== "admin" && event.organizerId !== session.user.id) notFound();
  ...
  const paidStats = orderStats[0] ?? { total: 0, count: 0 };
    ...
    <main className="mx-auto min-h-screen w-full max-w-[96rem] px-4 py-10 sm:px-6">
      ...
      <div className="mb-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <KpiCard
          ...
          label="Receita total"
          value={currency(paidStats.total)}
          sub={`${paidStats.count} pedidos pagos`}
          color="emerald"
        />
        <KpiCard
