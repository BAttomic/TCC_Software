...
export async function decrementSoldQuantity(ticketTypeId: string, quantity: number) {
  return (
    await TT.findOneAndUpdate(
      { _id: ticketTypeId, soldQuantity: { $lte: { $subtract: ["$totalQuantity", quantity] } } },
